...
#include <esp_timer.h>          // CHANGED: replaces sys/time.h
...
{
    return (uint32_t)esp_timer_get_time();  // CHANGED: replaces gettimeofday
}
...
{
    esp_rom_gpio_pad_select_gpio(dev->trigger_pin);
    esp_rom_gpio_pad_select_gpio(dev->echo_pin);
